    ...
    <div className="fixed z-40 bg-white">
      <div className="shadow-md flex justify-around gap-24 items-center h-16 w-screen">
        <figure className="h-16 w-auto z-20">
          <img src={LogoHorizontal || "/placeholder.svg"} alt="logo-header" className="h-full w-full object-cover" />
        </figure>
        ...
        <button className="block lg:hidden" onClick={toggleDropdownMenu}>
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-8 h-8 hover:rotate-180 transition all duration-300"
          >
      ...
      {dropdown ? (
        <div ref={dropdownRef} className="dropdown fixed top-30 mt-2 lg:hidden right-0 w-screen h-screen bg-white z-50">
          <nav className="gap-1 flex-col  text-xl w-full text-gray-700 flex items-center">
            <button
              className="w-full flex justify-end mb-5  hover:bg-gray-100 p-5"
              onClick={() => {
                setDropdown(false)
              }}
            >
              <img src={cancelIcon || "/placeholder.svg"} alt="" className="w-7 hover:scale-105" />
            </button>
            {currentUser ? (
              <>
                <NavLink
