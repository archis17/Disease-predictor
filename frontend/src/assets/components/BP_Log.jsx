const BP_Log = ({ responseData }) => {
    if (!responseData || !responseData.bp_log || !responseData.bp_log.date || responseData.bp_log.date.length === 0) {
      return <p className="h-full w-full grid place-content-center italic text-gray-500">Add your first value</p>
    }
    ...
    return (
      <div className="p-2 mb-1 rounded-lg">
        {responseData.bp_log.date.map((date, index) => {
          ...
          const isFirstValueOfDay = index === 0 || responseData.bp_log.date[index - 1] !== date
  
          return (
            <div key={index} className="flex flex-col mb-1">
              {isFirstValueOfDay && (
                <div className="flex items-center mb-2 bg-slate-200 p-1 rounded-md">
              ...
              <div className="ml-1">
                <div
                  className={`text-sm text-gray-700 border border-gray-200 rounded-md p-2 ${
