const GlucoseLevel = ({ responseData }) => {
    if (
      ...
      return <p className="w-full h-full grid place-content-center italic text-gray-500">No values in log</p>
    }
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="py-2 px-4 border-b font-semibold text-left">Before</th>
              <th className="py-2 px-4 border-b font-semibold text-left">After</th>
            </tr>
          </thead>
          <tbody>
            {responseData.blood_glucose.date.map((date, index) => {
              ...
              return (
                <tr key={index} className="border-b">
                  <td className={`py-2 px-1 border-r text-gray-800 ${isFirstDate ? "bg-sky-100" : "px-5"}`}>
                    <div className="flex items-center">
                      {isFirstDate && <div className="w-2 h-2 bg-gray-500 rounded-full mr-2"></div>}
                      <div>{date}</div>
                    </div>
                  </td>
                  <td className={`py-2 px-4 border-r ${getCellStyles(currentBefore, false)}`}>{currentBefore}</td>
                  <td className={`py-2 px-4 ${getCellStyles(currentAfter, true)}`}>{currentAfter}</td>
                </tr>
              )
            })}
          </tbody>
